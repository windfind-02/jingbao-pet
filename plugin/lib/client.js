window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const ANIMS = {
			...
			sleepy: "/pet_sleepy.webp?v=3", // 歪头瞌睡循环动图（15秒视频瞌睡段）
...
`;
		...
		function apply(ctx) {
			...
			function showConfirmBubble(info) {
				...
				bubbleText.textContent = pick(CONFIRM_LINES);
				bubbleActions.classList.remove("show");
				bubble.classList.add("show");
			...
			const mo = new MutationObserver((muts) => {
				// 弹窗被移除（如 ESC/遮罩关闭）→ 气泡消失 + 恢复计时器
				if (pendingConfirm && pendingConfirm.container && !document.contains(pendingConfirm.container)) {
					finishConfirm();
				}
